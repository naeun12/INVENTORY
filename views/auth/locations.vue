<template>
  <div class="d-flex" id="location">
    ...
    <div
      class="header-content w-100 d-flex flex-column bg-light mb-2"
    ...
    >
      ...
      <main>
          ...
          <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
              <div class="btn-group bg-light p-1 rounded-pill">
                ...
                >
                  All Locations
                  <span class="badge bg-white text-black ms-1" @click="CountLocations()">{{
                    countItem
                  }}</span>
                </button>
              </div>
              <button
                class="btn btn-blue text-white rounded-pill px-4 fw-bold shadow-sm"
        ...
        >
                ...
                <table class="table table-hover align-middle mb-0">
                  <thead class="bg-light">
                    <tr>
                      ...
                      <th class="py-3 text-uppercase x-small fw-bold text-secondary">Locations</th>
                  ...
                  <tbody>
                    <tr
                      v-for="(loc, index) in Locations.ListLocations"
                      :key="loc.locationID"
                      class="table-row-hover"
                    >
                      <!-- Numbering based on pagination -->
                      <td class="ps-4">#{{ loc.locationID }}</td>
          ...
          <ComponentPagination
            :total-locations="Locations.totalCount"
            :page-size="Locations.pageSize"
            :current-page="Locations.currentPage"
            :total-pages="Locations.totalPages"
            @go="fetchLocations"
...
export default {
  ...
  methods: {
    ...
    async CountLocations() {
      const response = await axios.get('https://localhost:5001/api/Location/countLocation', {
        headers: {
    ...
    async fetchLocations(page = 1) {
      try {
        ...
        const response = await axios.get('https://localhost:5001/api/Location/listLocations', {
          params: {
            pageNumber: page,
        ...
        this.Locations.ListLocations = data.items
    ...
    async saveLocation() {
      try {
        ...
        const response = await axios.post(
          'https://localhost:5001/api/Location/addLocations',
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
    ...
    async updateLocation() {
      ...
      try {
        ...
        formData.append('LocationName', this.currentLocation.locationName || '')
        const response = await axios.put(
          'https://localhost:5001/api/Location/editLocation',
          formData,
          {
            headers: {
    ...
    async getLocation(locationID) {
      this.loading = true
      try {
        const response = await axios.get(`https://localhost:5001/api/Location/getLocation`, {
    ...
    async deleteLocation() {
      ...
      try {
        const response = await axios.delete(
          `https://localhost:5001/api/Location/deleteLocation/${this.LocationID}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
