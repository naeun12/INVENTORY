      ...
      <main>
        ...
        <div class="card border-0 shadow-sm mx-4 my-3 p-3 rounded-4">
          <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            <ComponentSearch
              placeholder="Search Items or department heads..."
              v-model="search"
              @search="onsearch"
            />
        ...
        >
          <div class="table-responsive">
            <div class="mb-4 d-flex justify-content-end gap-3">
              <button
                class="btn btn-outline-primary rounded-pill px-4 shadow-sm fw-bold transition-all"
                @click="exportItems"
              >
          ...
          <ComponentPagination
            ...
            @go="fetchInventory"
            @next="fetchInventory(items.currentPage + 1)"
            @prev="fetchInventory(items.currentPage - 1)"
          />
        </div>
      </main>
    </div>
      ...
      <div class="modal-content border-0 shadow">
        ...
        <div class="modal-body">
          <form @submit.prevent="saveItem">
            ...
            <div class="mb-4">
              <label class="form-label small fw-bold text-secondary mb-2"> Item Image </label>
              <div
                class="upload-container d-flex flex-column align-items-center justify-content-center border-dashed rounded-3 p-4"
  ...
  methods: {
    ...
    async saveItem() {
      try {
        ...
        const response = await axios.post('https://localhost:5001/api/Item/addItems', formData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
    ...
    async updateItem() {
      ...
      try {
        ...
        const response = await axios.put('https://localhost:5001/api/Item/editItem', formData, {
          headers: {
    ...
    async deleteItem() {
      ...
      try {
        const response = await axios.delete(
          `https://localhost:5001/api/Item/deleteItem/${this.itemID}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
    ...
    async exportItems() {
      try {
        this.loading = true
        const response = await axios.get('https://localhost:5001/api/Item/exportItems', {
          responseType: 'blob',
        })
