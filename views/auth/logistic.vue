            ...
            <main class="container-fluid p-4">
                ...
                <div class="card border-0 shadow-sm mx-4 my-3 p-3 rounded-4">
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                        ...
                        <ComponentSearch placeholder="Search Items or Serial Number..." v-model="search"
                            @search="onsearch" />
                ...
                <div class="card border-0 shadow-sm mb-4 overflow-hidden" style="border-radius: 15px;">
                    <div
                        ...
                        <div class="bg-light p-1 rounded-pill d-flex gap-1 mt-3 mt-md-0"
                            style="border: 1px solid #e9ecef;">
                            <button @click="changeDirectory(false)"
                                :class="!IschangeDisplay ? 'bg-white shadow-sm text-primary' : 'text-muted border-0 bg-transparent'"
                                class="btn rounded-pill px-4 btn-sm fw-bold transition-base">
                                <i class="bi bi-grid-fill me-2"></i>Card
                            </button>
                            <button @click="changeDirectory(true)"
                                :class="IschangeDisplay ? 'bg-white shadow-sm text-primary' : 'text-muted border-0 bg-transparent'"
                                class="btn rounded-pill px-4 btn-sm fw-bold transition-base">
                ...
                <div v-if="items?.ListItems?.items?.length === 0"
                    ...
                    <div class="empty-state-container text-center px-4">
                        <div class="icon-box-glow mb-4 mx-auto d-flex align-items-center justify-content-center">
                            <i class="bi bi-search display-1 text-primary opacity-50"></i>
                        </div>

                        <h2 class="fw-bold text-dark">No Items Found</h2>
                        <p class="text-muted mb-4 mx-auto" style="max-width: 400px;">
                            Your directory is currently empty. Click the button below to start adding new products to
                            your logistics list.
                        </p>

                        <button class="btn btn-primary btn-lg rounded-pill px-5 shadow transition-base"
                            @click="refreshList">
                ...
                <div v-if="!IschangeDisplay">
                        ...
                        <div class="col" v-for="item in items.ListItems.items" :key="item.itemCode">
                            <div class="card h-100 shadow-sm border-0 product-card">
                                <div class="position-relative overflow-hidden group">
                                    ...
                                    <img :src="item.image || 'https://lookaside.fbsbx.com/lookaside/crawler/media/?media_id=100079694288900'"
                                        class="card-img-top p-2 rounded-4 img-fluid transition-zoom" alt="MISO Logo"
                    ...
                    <ComponentPagination :total-items="items.totalCount" :page-size="items.pageSize"
                        :current-page="items.currentPage" :total-pages="items.totalPages" @go="fetchItems"
                        @next="fetchItems(items.currentPage + 1)" @prev="fetchItems(items.currentPage - 1)" />
                </div>
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-centered">
                        <div class="modal-content border-0 shadow">
                            ...
                            <div class="modal-header border-bottom-0 pb-0">
                                ...
                                <button type="button" class="btn-close" @click="closeOrderModal"
                                ...
                                <div class="needs-validation">
                                    ...
                                    <div class="mb-2">
                                        <label class="form-label text-muted small fw-bold"><i
                                                class="bi bi-chat-left-text me-1"></i> PURPOSE /
                                            REMARKS</label>
...
export default {
    ...
    methods: {
        ...
        async listLocations() {
            try {
                const response = await axios.get('https://localhost:7050/api/Items/listLocations', {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                });
